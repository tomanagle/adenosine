...
func TestLoginServiceCompleteCleansUpCredentialFailures(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			grant := &fakeCredentialGrant{persistErr: test.persistErr}
			provider := &fakeProvider{
				identity: OAuthIdentity{DID: "did:plc:abcdefghijklmnopqrstuvwx"},
				grant:    grant,
			}
			sessions := &fakeSessions{err: test.localSessionErr}
			...
			if grant.persistCalls != 1 || grant.discardCalls != 1 || sessions.calls != test.wantSession {
