...
func TestIssueProjectionSQL(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "public list and counts use one bounded current projection query", required: []string{"-- name: GetNetworkIssueProjection :many", "repository.issue_count", "repository.open_issue_count", "issue.comment_count", "COALESCE(projected_issue.comment_count, 0) AS comment_count", "issue.deleted_at IS NULL", "issue.cid IS NOT NULL", "ORDER BY issue.record_created_at DESC, issue.uri DESC", "LIMIT sqlc.arg(page_size)"}},
