...
func TestProxyRejectsShapeOverridesAndInvalidContinuation(t *testing.T) {
	...
	testCases := []string{
		"offset=-1&table=core.accounts",
		"offset=-1&columns=uri%2Cpassword_hash",
		"offset=-1&where=TRUE",
		"offset=-1&secret=stolen",
		"offset=-1&subset__where=TRUE",
		"offset=-1&offset=0_0",
		"offset=-1&live=1",
		"offset=-1&replica=everything",
		"offset=-1&log=history",
		"offset=-1&cache-buster=" + strings.Repeat("x", 256),
		"handle=missing-offset",
	}
	for _, rawQuery := range testCases {
		t.Run(rawQuery, func(t *testing.T) {
			request := httptest.NewRequest(http.MethodGet, "/api/v1/sync/repositories?"+rawQuery, nil)
			if err := proxy.Forward(httptest.NewRecorder(), request); !errors.Is(err, ErrMalformed) {
				t.Fatalf("error = %v, want malformed", err)
			}
		})
	}
	post := httptest.NewRequest(http.MethodPost, "/api/v1/sync/repositories?offset=-1&live=true", nil)
	if err := proxy.Forward(httptest.NewRecorder(), post); !errors.Is(err, ErrMalformed) {
		t.Fatalf("POST-only allowlist error = %v, want malformed", err)
	}
}
...
func TestProxyPOSTSubsetValidation(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			request := httptest.NewRequest(http.MethodPost, "/api/v1/sync/repositories?offset=-1", strings.NewReader(tt.body))
			request.Header.Set("Content-Type", tt.contentType)
			if err := proxy.Forward(httptest.NewRecorder(), request); !errors.Is(err, tt.want) {
