...
func TestProxyOwnsRegisteredShapes(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "repositories", shape: Repositories, wantTable: "network.repositories", wantColumns: "uri,cid,owner_did,slug,name,description,default_branch,git_https,git_ssh,web,record_created_at,record_updated_at,indexed_at,star_count,issue_count,open_issue_count,comment_count,pull_request_count,open_pull_request_count", wantWhere: "deleted_at IS NULL AND cid IS NOT NULL", wantActor: "owner_did", wantRecord: "uri"},
