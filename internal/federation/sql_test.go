	...
	testCases := []struct {
	...
	}{
		{
			...
			required: []string{
				...
				"SELECT id FROM core.repositories WHERE at_uri = $1",
		...
		},
		{
			...
			required: []string{
				...
				"candidate.repository_uri = issue.repository_uri",
				"candidate.author_did = split_part(issue.repository_uri, '/', 3)",
		...
		},
		{
			...
			required: []string{
				...
				"candidate.author_did = split_part(pull_request.target_repository_uri, '/', 3)",
				"candidate.target_repository_uri = pull_request.target_repository_uri",
