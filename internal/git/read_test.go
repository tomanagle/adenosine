...
func TestBlobMetadataAndStreaming(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			metadata, err := fixture.service.BlobMetadata(context.Background(), fixture.id, test.sha)
			if err != nil {
				t.Fatalf("blob metadata: %v", err)
			}
			if metadata.SHA != test.sha || metadata.Type != "blob" || metadata.Size != int64(len(test.want)) {
				t.Errorf("metadata = %+v", metadata)
			}
			var output bytes.Buffer
			if err := fixture.service.StreamBlob(context.Background(), fixture.id, test.sha, &output); err != nil {
				t.Fatalf("stream blob: %v", err)
			}
			if !bytes.Equal(output.Bytes(), test.want) {
				t.Errorf("streamed bytes = %v, want %v", output.Bytes(), test.want)
			}
		})
	}
}
