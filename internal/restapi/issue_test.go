...
func TestIssueEndpoints(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "create accepts repository write PAT", method: http.MethodPost, path: "/api/v1/issues", body: `{"repository_uri":"` + restIssueRepositoryURI + `","title":"title","body":"body"}`, pat: true, manager: &fakeIssues{created: created}, wantStatus: http.StatusAccepted, wantCalls: 1, wantOperation: "create"},
