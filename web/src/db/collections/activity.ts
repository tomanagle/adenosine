...
export function composeRouteActivity(
  sources: RouteActivitySources,
  requestedLimit = 20,
): RouteActivityItem[] {
  ...
  for (const star of sources.stars ?? []) {
    items.push({
      ...
      uri: star.uri,
      cid: star.cid,
      actorDid: star.author_did,
      subjectUri: star.repository_uri,
      indexedAt: star.indexed_at,
    })
  }
  for (const issue of sources.issues ?? []) {
    items.push({
      ...
      uri: issue.uri,
      cid: issue.cid,
      actorDid: issue.author_did,
      subjectUri: issue.repository_uri,
      indexedAt: issue.indexed_at,
    })
  }
  for (const comment of sources.issueComments ?? []) {
    items.push({
      ...
      uri: comment.uri,
      cid: comment.cid,
      actorDid: comment.author_did,
      subjectUri: comment.issue_uri,
      indexedAt: comment.indexed_at,
    })
  }
  for (const pullRequest of sources.pullRequests ?? []) {
    items.push({
      ...
      uri: pullRequest.uri,
      cid: pullRequest.cid,
      actorDid: pullRequest.author_did,
      subjectUri: pullRequest.target_repository_uri,
      indexedAt: pullRequest.indexed_at,
    })
  }
  for (const review of sources.pullRequestReviews ?? []) {
    items.push({
      ...
      uri: review.uri,
      cid: review.cid,
      actorDid: review.author_did,
      subjectUri: review.pull_request_uri,
