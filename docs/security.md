...
The alpha does not claim comprehensive per-principal auth rate limiting, private federation, CI runners, LFS, enterprise SSO, or relay/custom-PDS support. Branch policies are enforced server-side for HTTP, SSH, and REST ref changes. Signed-commit policy trusts active account SSH keys and verifies every commit newly reachable from the protected branch; revoking a key removes it from future authorization without rewriting existing history. External CI is deliberately provider-neutral: signed webhooks trigger third-party systems, while repository-scoped `repository:status` tokens can report results but cannot read, administer, or write Git repositories. See the checklist for compensating operator controls and explicit residual risks. Report vulnerabilities using [`../SECURITY.md`](../SECURITY.md), not a public issue.
